//! navigator.clipboard object-shape probe (v0.8.63 M2).
//!
//! Verifies that navigator.clipboard is a non-null object with readText
//! and writeText Promise-returning methods. Promise rejection/value policy
//! deferred to v0.9+.
...
impl BehaviorProbe for ClipboardProbe {
    fn name(&self) -> &'static str {
        "navigator.clipboard Object Shape"
    }

    fn description(&self) -> &'static str {
        "Verifies navigator.clipboard is an object with readText and \
         writeText method properties and native code descriptor"
    }

    fn probe(&self, kernel: &mut EmbeddedV8Kernel) -> ProbeResult {
        match eval_str(kernel, "typeof navigator.clipboard") {
            Some(s) if s == "object" => {}
            v => {
                return ProbeResult::Fail {
                    reason: "navigator.clipboard is not an object".into(),
        ...
        for method in &["readText", "writeText"] {
            let expr = format!("typeof navigator.clipboard.{}", method);
            match eval_str(kernel, &expr) {
                Some(s) if s == "function" => {}
                v => {
                    return ProbeResult::Fail {
                        reason: format!("navigator.clipboard.{} is not a function", method),
...
mod tests {
    ...
    #[test]
    fn test_clipboard_probe_name_and_desc() {

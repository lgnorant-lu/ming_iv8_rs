...
#[test]
fn inner_html_basic() {
    let mut kernel = make_kernel_with_doc("<div id='x'><p>hello</p></div>");
...
fn inner_html_nested() {
    let mut kernel = make_kernel_with_doc("<div id='x'><ul><li>1</li><li>2</li></ul></div>");
    let result = kernel.eval_to_rust_value("document.getElementById('x').innerHTML");
    assert_eq!(
        result,
...
fn inner_html_with_attributes() {
    let mut kernel =
        make_kernel_with_doc("<div id='x'><a href=\"/link\" class=\"btn\">click</a></div>");
    let result = kernel.eval_to_rust_value("document.getElementById('x').innerHTML");
    match result {
        RustValue::String(s) => {
...
#[test]
fn outer_html_basic() {
    let mut kernel = make_kernel_with_doc("<div id='x'><p>hi</p></div>");
...
fn text_content_setter() {
    let mut kernel = make_kernel_with_doc("<div id='x'><p>old</p><span>stuff</span></div>");
...
fn inner_html_void_elements() {
    let mut kernel = make_kernel_with_doc("<div id='x'><br><img src=\"a.png\"></div>");
    let result = kernel.eval_to_rust_value("document.getElementById('x').innerHTML");
    match result {
        RustValue::String(s) => {

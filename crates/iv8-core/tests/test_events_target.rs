...
fn test_custom_event_is_trusted_false() {
    ...
    let val = common::to_str(&k.eval_to_rust_value("new Event('click').isTrusted"));
    assert_eq!(
        val, "false",
        "user-created events must have isTrusted=false"
    );
}
...
fn test_event_type_and_bubbles() {
    ...
    assert!(
        evt.contains("\"type\":\"custom\""),
        "event type wrong: {}",
        evt
    );
    assert!(
        evt.contains("\"bubbles\":true"),
        "event bubbles wrong: {}",
        evt
    );
}
...
fn test_prevent_default_exists() {
    let mut k = common::make_kernel();
    common::assert_js_str(&mut k, "typeof (new Event('x')).preventDefault", "function");
}

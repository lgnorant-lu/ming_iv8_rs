...
fn test_audio_context_create() {
    ...
    let val = common::to_str(&k.eval_to_rust_value(
        "var a = new AudioContext(); typeof a",
    ));
...
fn test_audio_context_destination_exists() {
    ...
    common::assert_js_str(
        &mut k,
        "typeof (new AudioContext()).destination",
        "object",
    );
}
...
fn test_audio_context_create_oscillator() {
    let mut k = common::make_kernel();
    let val = common::to_str(&k.eval_to_rust_value(
        "typeof (new AudioContext()).createOscillator()",
    ));
    assert_eq!(val, "object");
}
...
fn test_audio_context_create_gain() {
    let mut k = common::make_kernel();
    let val = common::to_str(&k.eval_to_rust_value(
        "typeof (new AudioContext()).createGain()",
    ));
    assert_eq!(val, "object");
}

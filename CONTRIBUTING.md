...
- rustfmt.toml 统一代码风格（**编译本身不跑 rustfmt/clippy**；需 `cargo fmt` / `cargo clippy` 或 hooks）
...
- **双仓路径**：若改动可能进入公仓，确认路径在 keep 名单内（见 3.6）

### 3.6 双仓路径门禁（每次提交 / 推送）

私仓 `_ming_iv8_rs` → 公仓 `ming_iv8_rs` 只导出 **keep 白名单** 路径。  
**文档提交同样适用**：新增 `docs/...` 若不在 keep 中，公仓不会有该文件。
...
# 可选严格：public-ish 却 DROP 则失败
# $env:IV8_STRICT_PUBLIC_PATHS = "1"

# 2) 若新增公仓应有路径：更新 manifests 后重建
# 编辑 scripts/public_sync/manifests/keep-top.txt（或 keep-scripts / keep-tools）
uv run python scripts/public_sync/build_keep_paths.py
```

**推送到 origin（私仓）前，若触及公仓表面路径：**

```powershell
# 完整 filter + LEAK（数分钟；与 CI dry-run 同工具）
pwsh -File scripts/public_sync/dry_run.ps1 -WorkRoot "$env:TEMP\opencode\public-export-check"
# 阅读 report\SUMMARY.md 与 leak-scan.md，HIGH 必须为 0
```

**Git hooks（推荐启用，见 `.githooks/README.md`）：**
...
| Hook | 默认 | 内容 |
|---|---|---|
| pre-commit | ON | `cargo fmt --check`（有 `.rs`）；`check_staged_paths` |
| pre-push | 触及公仓表面时 ON | 完整 `dry_run.ps1` |

**不要**把全量 `cargo test` / pytest 放进 hook（由开发/发布闸门负责）。  
跳过：`SKIP_IV8_HOOKS=1`；仅跳过 dry-run：`IV8_SKIP_PUBLIC_DRY_RUN=1`。
...
[ ] 相关测试通过（按改动范围；全量测试在发布/里程碑闸门，非每次 commit 强制）
[ ] 代码已格式化（cargo fmt）
[ ] clippy 无新增 warning（cargo clippy；lint job / 本地）
[ ] 没有 emoji（源码、文档、commit message）
[ ] commit message 符合 Conventional Commits 规范
[ ] subject 行 <= 72 字符，全英文
[ ] body 符合中英混排约定
[ ] git status 清洁（无意外文件）
[ ] 双仓：check_staged_paths 已看 KEEP/DROP；公仓应有路径已进 keep-top/scripts/tools
[ ] 若将 push 且改动公仓表面：dry_run PASS 或 pre-push 已跑通
```

...
- **Native Substrate Mainline (v0.8.17-v0.8.28)**: Navigator/Screen FunctionTemplate migration (descriptor.m1 184/184, fingerprint.m1 322/322), IDL preprocessing toolchain, Rust codegen + iv8-surface crate (1284 interfaces), BrowserSurface integration + Feature Flag removal, P0/P1 deep stubs (180+30 tests), infrastructure optimization (v8::Weak + cleanup), Feature Flag removal + user_overrides (v0.8.24), BCR upgrade + install_all Global-handle (v0.8.25), codegen scope-break + heap_limits GC fix + default path switch (v0.8.26), Phase C validation + archive + closure (v0.8.27), verification closure + BCR Step A Canvas2D/WebGL + L2 MVP spec (v0.8.28). 255/255 lib + 1284 Python + 81 Phase C tests PASS. v0.9 holding track continuing blocked.

Current `0.8.11` package release scope. v0.8.12-v0.8.28 are local milestones; package metadata remains `0.8.11`. Active mainline: [Native Substrate](docs/roadmap/post-v0.6/EXECUTION_ORDER.md). v0.9 holding track continuing blocked.

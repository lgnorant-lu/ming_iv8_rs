...
const EXCLUDED_ATTRIBUTES: &[(&str, &str)] = &[
    ...
    // --- Document tree accessors (owned by dom/binding.rs, RD-16/18) ---
    ("Document", "body"),
    ("Document", "documentElement"),
    ("Document", "head"),
    ("Document", "location"),
    // Element.innerText owned by DOM FT (template.rs); codegen HTMLElement
    ...
    // URL attrs owned by URL_SHIM_JS (real parse); codegen skeleton empty.
    ("URL", "href"),
    ("URL", "origin"),
    ("URL", "protocol"),
    ("URL", "username"),
    ...
    ("URL", "hash"),
    ("URL", "searchParams"),
    // Table/Dialog attrs owned by DOM FT (template.rs) — empty codegen skeletons TYPE_FAIL H05a.
    ...
    // Node.baseURI and Node.ownerDocument are no longer excluded.
    // They are generated by codegen and copied to dom_proto by
    // chain_dom_prototypes' property copy loop. DOCUMENT_PROPS_JS
    // also installs instance-level overrides on the document object,
...
const EXCLUDED_OPERATIONS: &[(&str, &str)] = &[
    ...
    // Element tree-backed (dom/template.rs) — RD-06
    ...
    // Canvas (template get_context_cb / to_data_url_cb) — RD-20
...
fn should_skip_operation(interface_name: &str, op_name: &str) -> bool {
    EXCLUDED_OPERATIONS
        .iter()
        .any(|(iface, op)| *iface == interface_name && *op == op_name)
}
...
fn generate_domain_file(
    ...
    for def in defs {
        ...
        for m in &def.members {
            ...
            // Skip if the prototype already owns this property (DOM template
            // accessors must not be overwritten by codegen fix_accessors).
            out.push_str("                        let __existing = proto_obj.get_own_property_descriptor(scope, attr_key.into());\n");
            out.push_str("                        if __existing.is_none() || __existing.is_some_and(|d| d.is_undefined()) {\n");
...
fn generate_callbacks(def: &Definition, fn_name: &str, enum_names: &std::collections::HashSet<String>) -> String {
    ...
    for m in &def.members {
        if m.kind == "attribute" {
            ...
            // AbortController.signal: cache created AbortSignal on controller.
            if iface_name == "AbortController" && attr_name == "signal" {
                out.push_str(
                    r#"        {
                ...
                if __ctor_val.is_function() {
                    ...
                    if let Some(__proto) = __ctor.get(scope, __proto_key.into()) {
                        if __proto.is_object() {
                            ...
                            let _ = __this.set(scope, __hidden_key.into(), __obj.into());
                            rv.set(__obj.into());
                            return;
                        }
                    }
...
"#,
        ...
        if m.kind == "operation" {
            let op_name = m.name.as_deref().unwrap_or("");
            if should_skip_operation(def.name.as_deref().unwrap_or(""), op_name) {
                continue;
            }
            idx += 1;
            ...
            // v0.8.92: AbortController.abort marks signal.aborted (native, not JS patch).
            if iface_name == "AbortController" && op_name == "abort" {
                out.push_str(
                    r#"    // AbortController.abort(): set signal.__iv8Aborted = true; cache signal.
    let __this = __args.this();
    let __sig_key = v8::String::new(scope, "__iv8Signal").unwrap();
    let __signal_obj = if let Some(existing) = __this.get(scope, __sig_key.into()) {
        if existing.is_object() && !existing.is_null_or_undefined() {
            existing.to_object(scope)
        } else {
            None
        }
    } else {
    ...
    let __signal_obj = __signal_obj.or_else(|| {
        let __ctor_name = v8::String::new(scope, "AbortSignal").unwrap();
        let __global = scope.get_current_context().global(scope);
        if let Some(__ctor_val) = __global.get(scope, __ctor_name.into()) {
            if __ctor_val.is_function() {
                let __ctor = unsafe { v8::Local::<v8::Function>::cast_unchecked(__ctor_val) };
                let __proto_key = v8::String::new(scope, "prototype").unwrap();
                if let Some(__proto) = __ctor.get(scope, __proto_key.into()) {
                    if __proto.is_object() {
                        let __obj = v8::Object::new(scope);
                        let _ = __obj.set_prototype(scope, __proto);
                        let _ = __this.set(scope, __sig_key.into(), __obj.into());
                        return Some(__obj);
                    }
                }
            }
    ...
    if let Some(sig) = __signal_obj {
        let __aborted_key = v8::String::new(scope, "__iv8Aborted").unwrap();
        let __true = v8::Boolean::new(scope, true);
        let _ = sig.set(scope, __aborted_key.into(), __true.into());
    }
...
"#,
                ...
                let ret_nullable = ret_name.ends_with('?');
                if enum_names.contains(bare_ret) {
                    out.push_str("    rv.set(v8::String::new(scope, \"\").unwrap().into());\n");
                } else if is_interface_type(ret_name) {
                    // Nullable interface returns (T?): honest default is null, not a
                    // fake empty T and never look up ctor name "T?" (broken).
                    if ret_nullable {
                        out.push_str("    rv.set(v8::null(scope).into());\n");
                    } else {
                        out.push_str(&format!(

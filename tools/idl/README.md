...
- `parse-chromium-idl.js` is optional input to merge: write `tmp/chromium.ir.json` or pass `--chromium-ir <path>`. Default pipeline still uses `chrome-manual/*.ir.json` only.

...
**PyPI：** `ming_iv8_rs` · **import：** `import iv8_rs` · **包轨版本：** `0.8.12`（D-151；与 continuum 里程碑 `v0.8.10x` 编号可不相同）  
...
需要 **Python 3.13+**。ICU **77** 数据随包提供（`icudtl.dat`；可用 `IV8_ICUDTL_PATH` 覆盖）。

### 预编译 wheel（PyPI / GitHub Release）
...
| 平台 | 典型 tag | 说明 |
|---|---|---|
| Windows x86_64 | `win_amd64` | 主桌面 |
| Linux x86_64 | `manylinux_2_34_x86_64` | V8 150+ 预编译（cdylib TLS 安全） |
| macOS Apple Silicon | `macosx_*_arm64` | 仅 arm64 |

**0.8.12 不提供：** macOS Intel、Linux aarch64（需源码自建）。

GitHub 二进制 Release 标签（若创建）：**`pkg-0.8.12`**（包轨发布；**不是** continuum 里程碑 `v0.8.12`）。

### 源码构建

另需 **Rust 工具链**（见 workspace `rust-version`）。
